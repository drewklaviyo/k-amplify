...
export interface RawProject {
  ...
  externalLinks: { id: string; url: string; label: string | null; createdAt: string }[];
}
  ...
  team: {
    projects: {
      nodes: {
        ...
        externalLinks: { nodes: { id: string; url: string; label: string | null; createdAt: string }[] };
      }[];
      pageInfo: { hasNextPage: boolean; endCursor: string | null };
    };
  } | null;
  ...
  while (true) {
    ...
    for (const node of page.nodes) {
      allProjects.push({
        ...
        externalLinks: node.externalLinks.nodes,
      });
    }
